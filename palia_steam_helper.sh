...
class OnlineZip(ZipFile):
    ...
    def _support(self):
        req: Request = Request(self.url, method="HEAD", headers={'User-agent': 'PaliaSteamHelper/1.00.0'})
    ...
    def open(self, name, mode="r", pwd=None, *, force_zip64=False):
        ...
        try:
            ...
            file_stream = self._stream_bytes(file_info.header_offset + self.cd_start + sizeFileHeader,
...
def download(url, target_path):
    curl = Popen(['curl', '--progress-bar', '--retry', '5', '--retry-max-time', '120', '-L', '-o', target_path, '-C', '-', url])
    return 0 if curl.returncode == None else curl.returncode
    ...
    if filename == None:
        for entry_name in z.namelist():
            ...
            if entry_name[-1] != '/':
                ...
                if ( Path(f"{target_path}/{entry_name}").exists() and KNOWN_HASHES.get(entry_name, "") != ""):
                    logging.debug(f"Download/Unzip - File exists, validating it: {entry_name}")
                    if fsize > 128000000:
                        zenity.stdin.write(f"# {text}\\nFile exists, validating it...\\n{Path(entry_name).name}\\nOverall progress: {prog}%\n")
                        zenity.stdin.write(f"{prog}\n")
                        zenity.stdin.flush()
                    if file_hash(f"{target_path}/{entry_name}") == KNOWN_HASHES.get(entry_name, ""):
                        i.close()
                        logging.debug(f"Download/Unzip - File valid, skipping it: {entry_name}")
                ...
                if entry_name in KNOWN_HASHES:
                    ...
                    zenity.stdin.write(f"{prog}\n")
                    zenity.stdin.flush()
                    if KNOWN_HASHES.get(entry_name, "") == file_hash(f"{target_path}/{entry_name}"):
                        logging.debug(f"Validated after downloading: {entry_name}")
                    else:
                        logging.debug(f"Failed validation after downloading: {entry_name}")
...
def replace_file(filename, prog_text = None, zeni = None):
    ...
    if ( url == "" ):
        ...
        if ( kh == "" ):
            logging.debug(f"Unknown file, adding to hashes: {get_hash_file_key(filename)}")
            return
        else:
            logging.debug(f"Must get from zip: {get_hash_file_key(filename)}")
            extract(PALIA_BASE_VER_URL, PALIA_CLIENT_PATH, "Replacing File", f"Downloading and extracting...\\n{Path(filename).name}", get_hash_file_key(filename))
            return

    Path(filename).parent.mkdir(parents=True, exist_ok=True)
    drc = download_progress(url, filename, "Downloading...", f"{prog_text}{Path(filename).name}", zeni)
    if ( drc == 0 ):
...
def validate_hashes(hashes, title, text):
    ...
    for f in hashes.keys():
        fcount = fcount + 1
        zenity.stdin.write(f"# {text}{ round(float(fcount)/float(len(hashes)) * 100,1) }%\\nChecking file:\\n{Path(f).name}\\nPlease wait...\n")
        ...
        if ( not validate_file(f) ):
            if zenity.poll() != None:
                sys.exit(zenity.returncode)
            if replace_file(f, f"{text}{ round(float(fcount)/float(len(hashes)) * 100,1) }%\\nDownloading file:\\n", zenity) == 1:
                zenity.terminate()
                sys.exit()
    zenity.terminate()
...
def validate_all_files():
    ...
    if len(KNOWN_HASHES) <= 2:
        ...
        #get_base_zip()
        download_progress(f"{PSH_REPO}/{KNOWN_HASH_FILE}", KNOWN_HASH_FILE, "Palia Known Hashes", "Downloading known hashes from Github...")
        load_known_hashes()
...
def get_base_zip():
    ...
    rs = extract(PALIA_BASE_VER_URL, PALIA_CLIENT_PATH, "Installing Palia", "Downloading and extracting base game files...")
    if rs != 0:
        sys.exit(rs)
